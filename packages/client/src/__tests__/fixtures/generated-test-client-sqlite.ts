 ...
 *
 * Generated at: 2026-09-02T20:43:29.864Z
 */
...
function buildFieldExpression_User(
  ...
  const comparisonField = comparisonCast_User[field] === true
    ? sql`cast(${sql.ref(`User.${field}`)} as integer)`
    : qualifiedField
...
class UserOperations {
    ...
    if (args?.orderBy) {
      ...
      for (const orderByObj of orderByArray) {
        Object.entries(orderByObj).forEach(([field, direction]) => {
          ...
          query = comparisonCast_User[field] === true
  ...
  private prepareCreateData(data: UserCreateInput): Record<string, unknown> {
    ...
    if (data.createdAt !== undefined) {
      prepared.createdAt = data.createdAt instanceof Date ? data.createdAt.toISOString() : new Date(data.createdAt as string | number).toISOString()
    }
    
    if (data.publishedAt !== undefined) {
      prepared.publishedAt = data.publishedAt instanceof Date ? data.publishedAt.toISOString() : new Date(data.publishedAt as string | number).toISOString()
    }
    
    if (data.createdAt === undefined) { prepared.createdAt = new Date().toISOString() }
    if (data.publishedAt === undefined) { prepared.publishedAt = new Date().toISOString() }
